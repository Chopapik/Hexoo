create or replace function public.moderator_block_user_tx(
  p_uid_to_block text,
  p_banned_by text,
  p_banned_reason text
) returns void
language plpgsql
as $$
begin
  if p_uid_to_block is null or btrim(p_uid_to_block) = '' then
    raise exception 'uid_to_block is required';
  end if;
  ...
  set
    ...
    banned_reason = p_banned_reason,
    updated_at = now()
  where uid = p_uid_to_block;

  if not found then
    raise exception 'User % not found', p_uid_to_block;
  end if;
  ...
  values (
    p_uid_to_block,
    'USER_BLOCKED',
    format('Blocked by %s. Reason: %s', coalesce(p_banned_by, 'unknown'), coalesce(p_banned_reason, ''))
  );
  ...
  values (
    ...
    format('Blocked user %s', p_uid_to_block)
  );
end;
$$;

create or replace function public.moderator_unblock_user_tx(
  p_uid text,
...
begin
  ...
  insert into public.activity_logs (user_id, action, details)
  values (p_uid, 'USER_UNBLOCKED', 'User account unblocked');

  insert into public.activity_logs (user_id, action, details)
  values (
    p_moderator_uid,
    'MODERATOR_UNBLOCKED_USER',
    format('Unblocked user %s', p_uid)
  );
end;
$$;
...
begin
  ...
  return jsonb_build_object(
    'deletedImagePath',
    case when p_action = 'reject' then v_post.image_meta ->> 'storagePath' else null end,
    'authorUid',
    v_post.user_id
  );
end;
