...
export const Header = ({ user }: { user: SessionData | null }) => {
  ...
  return (
    <div className="flex h-[100px] w-full items-center justify-between overflow-hidden border-b border-surface-chrome-border-default bg-surface-chrome-background-default px-8 backdrop-blur-[16px] md:h-auto md:justify-center md:px-4 md:py-2">
      <div className="flex min-w-0 flex-1 items-center justify-between md:max-w-[1350px]">
        ...
        <div className="flex shrink-0 items-center">
          {user ? (
            <Link
              href={`/profile/${encodeURIComponent(user.uid)}`}
            ...
            >
              <Avatar
                ...
                alt={user.name}
                className="size-12 rounded-xl md:size-10"
              />
