...
function readPost(client: QueryClient, key: readonly unknown[]) {
  return client.getQueryData<InfiniteData<PublicPostResponseDto[]>>(key)
    ?.pages[0][0];
}
...
describe("useToggleLike multi-cache contract", () => {
  ...
  it.each([
    ...
    "reconciles liked=%s across feed and every user-post cache",
    async (liked, likesCount) => {
      ...
      await waitFor(() =>
        expect(fetchClient.post).toHaveBeenCalledWith("/posts/post-1/like", {
          liked,
        }),
      );
      await waitFor(() => {
  ...
  it("rolls back every touched cache when the API fails", async () => {
    ...
    const before = [feedKey, ownerKey, viewerKey].map((key) =>
      readPost(client, key),
    );
    const { result } = renderHook(() => useToggleLike(), {
      wrapper: wrapper(client),
  ...
  it("uses the backend response rather than the optimistic count", async () => {
    ...
    await waitFor(() =>
      expect(readPost(client, feedKey)).toMatchObject({
        isLikedByMe: true,
        likesCount: 9,
      }),
    );
